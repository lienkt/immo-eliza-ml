# Immo Eliza ML: Project Documentation

## Description

Following our data scraping phase, cleaning, exploring, and analyzing the Belgian real estate dataset for **Immo Eliza**. The ultimate goal od this project is to create a machine learning model to predict prices of real estate properties in Belgium.
...
Apply the same steps to the new dataset later on when generating predictions
...
3. clean dataset
   ↓
4. main.py (choose model)
   ↓
5. trainning_model.py
   ↓
6. models (.pkl)
...
df → preprocess → split → train → evaluate → save
...
_Note: All columns are structured in lower_case. Missing or blank values are strictly set to `null` or `NaN` during the cleaning phase._# immo-eliza-turtles-analysis

# 🏠 Immo Eliza - Machine Learning Regression Project

This project builds a machine learning system to predict real estate prices in Belgium using multiple regression models.

We implemented a full ML pipeline including data preprocessing, model training, evaluation, comparison, and model saving.
...
Each model is trained using a reusable pipeline:
...
- Underfitting (low train R²)

## 4. Model Comparison

All models are compared and the best model is selected based on test R² score.
...
Models are saved as full pipelines including preprocessing.
...
We created a core function `_train_model()` to avoid code duplication across models.

## ✔ Consistent Evaluation
All models use the same evaluation function for fair comparison.
...
Student project - Machine Learning Regression Pipeline
```
